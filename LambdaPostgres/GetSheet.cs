    ...
    public class GetSheet
    {
        public async Task<List<Character>> FunctionHandler(ILambdaContext context)
        {
            ...
            try
            {
                ...
                await using var cmd = new NpgsqlCommand("SELECT ch.class, ch.level, ch.race, ch.name, c.ac, c.initiative, c.passive_perception, c.hit_dice, c.strength, c.dexterity, c.constitution, c.intelligence, c.wisdom, c.charisma, c.current_pf FROM character ch JOIN creature c ON c.id=ch.id;", conn);

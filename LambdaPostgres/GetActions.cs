    ...
    public class GetActions
    {
        public async Task<List<Action>> FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
        {
            ...
            try
            {
                ...
                string query = @"SELECT a.name, a.description FROM action a JOIN action_pg apg ON a.name = apg.action JOIN character ch ON apg.creature = ch.id WHERE ch.name = @charName;";
